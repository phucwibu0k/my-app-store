...
def _build_entry(rel: dict, tag: str, ipa_info: dict,
    ...
    """Tạo dict metadata cho một release (đã thêm direct_install)."""
    body = rel.get('body', '')
    return {
        "name":           ipa_info['name'],
        "id":             ipa_info['id'],
        ...
        "direct_install": parse_direct_install(body),   # ← THÊM MỚI
...
def update_metadata_from_releases() -> bool:
    """
    Lấy GitHub Releases, xử lý từng release còn mới và cập nhật app_metadata.json.
    """
